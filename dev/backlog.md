...
[P2][done] Section terminés en bas du viewer (SA17) — sortir les done du flux, section dédiée triée par date de clôture ~1.5h =1h @epic:dx
  > Fait (DEV) : build.py — les done sortent du flux principal vers une section « Terminés » en bas (header + compteur) ; actifs (todo/progress/blocked) en haut. Prouvé : 27 tests parse() OK, séparation vérifiée (0 done en haut, 0 todo en bas). Refactor minimal (corps de rendu inchangé). Tri « par date de clôture » NON fait : aucun champ date dans le format → décision (ajouter un champ = maj convention+parser) ; section triée comme le reste (epic+prio) en attendant.
[P1][done] Nettoyage sûr des branches et worktrees (SA18) — avant toute suppression, git log main..branche ; si non vide STOP + surfacer ; ne retirer que le worktree ~2h =0.5h @epic:core
  > Fait (DEV) : WORKFLOW.md §5 réécrit « nettoyage SÛR » — garde-fou git log main..branche (STOP si non-vide), worktree remove (sûr : garde les commits), branch -d (refuse une branche non fusionnée, jamais -D à l'aveugle). Générique, livrable public. Prouvé : une branche non intégrée est listée par git log main..branche et refusée par git branch -d.
[P2][done] Pré-sync repo→hub : rendre dev/backlog.md lossless (SA19) — porter ici les tickets hub-only (ci-dessus) ; vérifier le mapping de convention ; ajouter au DEV.md le garde-fou changelog atomique (RUNLOG commité avec le code) ~2h =1h @epic:core
  > Fait (DEV) : 3 volets clos — (1) tickets hub-only portés dans dev/backlog.md (commit 2dcaeb4) ; (2) mapping de convention vérifié (parsing OK, 38 tickets, aucune chip parasite #infra) ; (3) DEV.md §5 « Doc & journal dans le même commit » : changelog/RUNLOG commité atomiquement avec le code qu'il décrit. Générique, livrable public.
